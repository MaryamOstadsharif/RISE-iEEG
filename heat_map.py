...
def plot_hist_seperate(type, num_best_elec):
    ...
    for patient in range(len(ig)):
        for event in range(event_id_f, len(ig[0])):
            ...
            pos_elec_best[patient, event - event_id_f] = pos
            label_elec_best[patient, event - event_id_f] = ch[patient][pos][7:]
        count_label_singing.extend(np.array(Counter(label_elec_best[patient, :16]).most_common())[0:num_best_elec, 0].tolist())
        count_label_music.extend(np.array(Counter(label_elec_best[patient, 16:]).most_common())[0:num_best_elec, 0].tolist())
...
def plot_best_elec():
    ...
    if ch[patient][0][4] == 'r':
        ...
        plotting.view_markers(coor_best_elec_right)
