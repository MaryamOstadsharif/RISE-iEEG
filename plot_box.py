...
models = []
data = {}
# 2024-05-12-02-51-36 for S/M
# 2024-05-09-20-17-05 for Move/rest
if compare:
...
if compare_type == 'patient':
    data_one = np.load(
        'F:/maryam_sh/General model/General code/results/' + task + '/' + balancing + '/2024-05-04-11-45-22/accuracy'
                                                                                      '/fscore_' + models[0] +'each_patient_' +str(
            num_fold) + '.npy')
...
if task == 'Singing_Music' or task == 'move_rest':
    ax.set_xticklabels(['R', 'HTNet', 'EEGNet', 'RandomForest', 'MinimumDistance'], fontsize=14)
else:
