...
class MultiPatient_model:
    ...
    def save_result(self):
        ...
        np.save(self.sp + 'precision_GNCNN_each_patient' + '_' + str(self.n_folds) + '.npy', self.precision_patient_folds)
    ...
    def train_evaluate_model(self, X_train_all, y_train_all, X_val_all, y_val_all, X_test_all, y_test_all, chckpt_path):
        ...
        # design GNCNN model
        model = GNCNN(nb_classes=len(np.unique(np.argmax(y_train_all, axis=1))),
        ...
        num_test = X_test_all[0].shape[0] / self.num_patient
        for i in range(self.num_patient):
            preds = model.predict(
                [X_test_all[k][i * int(num_test):(i + 1) * int(num_test), :, :, :] for k in range(self.num_patient)]).argmax(
                axis=-1)
    ...
    def load_split_data(self, lp, n_chans_all, type_balancing,rand_seed=1337):
        ...
        data_all_input, labels = load_data(self.num_patient, lp, n_chans_all=n_chans_all, task=self.task)
